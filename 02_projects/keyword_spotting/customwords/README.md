...
The scope in this project is to train and quantize a Tensorflow Lite Micro model and deploy it to the Arduino Nano 33 BLE Sense. The model should enable the Arduino to recognize and to respond to the four custom (self recorded) keywords "activate" , "deactivate" and "snapshot". If the controller detects "activate" it turns on the green led. If it detects the keyword "snapshot", it activates the white led. If it detects "deactivate" the red led is activated. 
